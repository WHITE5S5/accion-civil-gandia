import { cacheHeaders, getActuaciones } from './lib/content-api.mjs';
...
export default async (req) => {
  ...
  const data = await getActuaciones({ ...Object.fromEntries(url.searchParams.entries()), slug: url.searchParams.get('slug') });
  const item = data.items[0] || null;
  return new Response(JSON.stringify({ ok: true, source: data.source, item }), { status: item ? 200 : 404, headers: cacheHeaders() });
};

export const config = { path: '/api/actuacion-detail' };
