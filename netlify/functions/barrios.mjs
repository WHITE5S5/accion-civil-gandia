import { cacheHeaders, getBarrios } from './lib/content-api.mjs';
...
export default async (req) => {
  ...
  const data = await getBarrios(Object.fromEntries(new URL(req.url).searchParams.entries()));
  return new Response(JSON.stringify(data), { status: 200, headers: cacheHeaders() });
};

export const config = { path: '/api/barrios' };
